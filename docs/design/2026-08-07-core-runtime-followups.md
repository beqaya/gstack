...
| A parked approval request could not be corrected once written | The founder's list could go stale with no way to fix it: a second `park` was exit 8, `resolve` closed the request rather than changing it, and `claim` would not hand the item back. It bit the run that found it — item `2b7045702f` asked approval to push *three* commits and two more landed while it waited. Closed by `amend` (`ce0f9b31`): `parked_state` now decides by ORDER, so a park filed after a decision returns the item to `awaiting`. **Founder ruling: amending an already-decided request resets the decision** — a request whose substance changed after it was answered has not been answered. Review attacked the state machine across five interleaving and re-decision sequences with no double-count or drop. The original stale record was then corrected with the command built for it. |
...
| A decision record with no matching park is now invisible | `parked_state`'s `decisions` map requires the item to have a park record, so a hand-written or partially-corrupted `parked.jsonl` holding a lone `{"decision": …}` line drops it from both lists — it previously counted as resolved. Unreachable through the CLI: `resolve` refuses an item that is not awaiting (exit 18). Deliberate behaviour change, recorded because it is a change: dropping a decision that resolves nothing is the safer reading. |
| The 25-character floor penalises information-dense scripts | `已运行命令并比较输出，结果与规范一致` — a complete sentence reporting a command run and an output compared — is refused at 18 characters, and passes once padded to 31. Pre-existing in `reject_thin`, but now the *only* remaining source of false positives in the whole gate (2 of 46 in the reviewer's corpus, both this floor). A character count treats one CJK character as worth one Latin letter, which it is not. Arabic is unaffected — its word lengths are comparable to English. |
