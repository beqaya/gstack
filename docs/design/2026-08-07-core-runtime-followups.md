...
| The gate caught typed stand-ins but not vacuous prose | Also closed at the founder's direction. Two rules: `VACUOUS` refuses prose that concludes without observing (`looks good`, `works as intended`, `I am confident`) in claim, evidence, action and reason; `OBSERVED` requires evidence for a **PROVEN** verdict to cite something re-derivable — an action taken, a number, a path, or a quoted fragment. Deliberately not applied to `UNPROVEN`, which exists to record that evidence could not be shown. Both mutation-verified independently. The residual limit is real and stated in the skill: a *fabricated* specific still passes, which is what the independent verifier is for. |
