...
```bash
~/.claude/skills/gstack/bin/gstack-playbook render sync-gbrain 2>/dev/null || true
```
...
If B: continue to Step 3.5 (not Step 4) with the empty-corpus state recorded — Step 3.5
still needs to run to compute `CYCLE` for Step 5's verdict block, even though its own
`PAGES > 0` condition means it won't prompt here.
