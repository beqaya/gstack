...
export function resolveStartTimeout(env: NodeJS.ProcessEnv = process.env): number {
  ...
  // 45s: first-launch Chromium on Windows measured past 15s (false-failure reports).
  const platformDefault = IS_WINDOWS ? 45000 : (env.CI ? 30000 : 15000);
...
async function ensureServer(flags?: GlobalFlags): Promise<ServerState> {
  ...
  if (process.env.BROWSE_NO_AUTOSTART === '1') {
    ...
    console.error('[browse] The headed browser may have been closed. Run /connect-chrome to restart.');
    process.exit(1);
  }
  ...
  if (state && state.mode === 'headed' && isProcessAlive(state.pid)) {
    ...
    console.error(`[browse] Run '/connect-chrome' to restart.`);
    process.exit(1);
  }
