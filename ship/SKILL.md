...
Use when asked to "ship", "push to main",
...
is ready, wants to push code up, or asks to create a PR.
...
```bash
~/.claude/skills/gstack/bin/gstack-playbook render ship 2>/dev/null || true
```
...
1. **Classify state** — pure reader, never writes:
   ...
   - **DRIFT_STALE_PKG** → run `~/.claude/skills/gstack/bin/gstack-version-bump repair` (syncs package.json to VERSION). No re-bump; reuse `currentVersion` for CHANGELOG + PR.
