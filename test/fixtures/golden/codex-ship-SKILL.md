...
```bash
$GSTACK_BIN/gstack-playbook render ship 2>/dev/null || true
```
...
If `total` is 0, skip this step silently. Continue to Step 11 — the adversarial review is diff-based, not PR-based, and always runs whether or not a PR exists yet.
...
1. **Classify state** — pure reader, never writes:
   ...
   - **DRIFT_STALE_PKG** → run `$GSTACK_BIN/gstack-version-bump repair` (syncs package.json to VERSION). No re-bump; reuse `currentVersion` for CHANGELOG + PR.
...
3. **Queue-aware pick** (workspace-aware ship):
   ...
   if command -v jq >/dev/null 2>&1; then
     ...
     NEW_VERSION=""  # no jq — force the offline fallback below instead of dying on an empty version
   fi
   ```
   If `offline`/util fails **or `NEW_VERSION` is empty**: fall back to local `BUMP_LEVEL` arithmetic and print `⚠ workspace-aware ship offline — using local bump only`. If `claimed` is non-empty, render the queue table so the user sees landing order. If an active sibling workspace holds a version `>= NEW_VERSION`, **AskUserQuestion**: advance past (unrelated work) or abort and sync with the sibling.
...
**If the subagent fails or returns invalid JSON:** Print a warning and proceed to Step 18.5 without a `## Documentation` section. Do not block /ship on subagent failure. The user can run `/document-release` manually after the PR lands.

---

## Step 18.5: Outcome gate (mandatory, before the PR/MR is created or updated)

Before the PR/MR body is composed, invoke `/verify-outcome` for the user-visible
thing this change was supposed to make true. A `PROVEN` verdict is required to
proceed to Step 19. A green CI run, a successful deploy, and an HTTP 200 are
explicitly not substitutes (see the 2026-08-03 blank-page outage, where all
three were green while the site rendered nothing).

**On `UNPROVEN` or `CONTRADICTED`: STOP.** Do not create or update the PR/MR —
the code stays pushed (Step 17 already ran) but nothing gets opened for review
until the outcome is proven. Report exactly what evidence was missing. Do NOT
describe the work as shipped, and do NOT continue to Step 19.

This runs here — before Step 19 — specifically so an UNPROVEN or CONTRADICTED
verdict can never reach a PR title, body, or URL the user has already seen and
started to trust. Running the same check after the PR exists is too late to
affect the PR.
...
## Step 22: Outcome gate — already enforced at Step 18.5

The mandatory outcome gate (`/verify-outcome`) now runs at Step 18.5, inside
`sections/pr-body.md`, **before** the PR/MR is created or updated — not here.
Running it this late (after the PR URL is already in front of the user) was
the defect: a CONTRADICTED verdict could no longer affect the PR. See Step
18.5 for the full gate logic; do not re-run or duplicate it here.

If you reached this line, Step 18.5 already returned `PROVEN` — report success
now. If it had returned `UNPROVEN` or `CONTRADICTED`, Step 18.5's STOP fired
before Step 19 and this line was never reached.
