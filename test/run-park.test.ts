...
describe('gstack-run park', () => {
  ...
  test('parking an already-completed item is refused', () => {
    ...
    run(['claim', '--run', runId, '--worker', 'w1'], root);
    run(['journal', '--tier', 'routine', '--run', runId, '--item', item, '--claim', 'the item behaves as specified after the change', '--verdict', 'PROVEN', '--evidence', 'ran the command and observed the documented exit code and output'], root);
