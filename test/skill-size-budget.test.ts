 ...
 *
 * The baseline lives at test/fixtures/parity-baseline-v1.65.0.0.json,
 * re-captured 2026-09-12 when the fork absorbed upstream's skill-start runtime
 * (the v1.71.0.0 bash consolidation; upstream's own v1.69.1.0 capture is kept
...
const BASELINE_PATH = path.join(REPO_ROOT, 'test', 'fixtures', 'parity-baseline-v1.65.0.0.json');
...
describe('SKILL.md size budget regression (gate, free)', () => {
  test('parity-baseline-v1.65.0.0.json exists', () => {
    expect(fs.existsSync(BASELINE_PATH)).toBe(true);
  });

  test('no skill exceeds v1.65.0.0 baseline size × ratio', () => {
   ...
   */
  test('no skill shrinks past 80% of v1.65.0.0 baseline (catches accidental body strip)', () => {
    const baseline: ParityBaseline = JSON.parse(fs.readFileSync(BASELINE_PATH, 'utf-8'));
    const current = captureBaseline({ repoRoot: REPO_ROOT });
