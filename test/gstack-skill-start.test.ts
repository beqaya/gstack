...
// The script forks ~30 helper processes; through Git Bash on Windows that is
// 20-40s, not the 5s bun default.
if (process.platform === 'win32') setDefaultTimeout(120_000);
...
// Windows cannot exec a shebang script directly; route through bash there.
function runScript(file: string, args: string[], opts: Parameters<typeof execFileSync>[2]): string {
  return process.platform === 'win32'
    ? (execFileSync('bash', [file, ...args], opts) as string)
    : (execFileSync(file, args, opts) as string);
}

function runStart(args: string[] = [], env: Record<string, string> = {}): string {
  return runScript(START, ['--skill', 'testskill', ...args], {
    encoding: 'utf-8',
...
describe('gstack-skill-start behavior', () => {
  test('sanitizes GSTACK_INSTRUCTION markers out of passthrough output (OV4)', () => {
    ...
    try {
      ...
      const out = runScript(path.join(fakeBin, 'gstack-skill-start'), ['--skill', 't'], {
        encoding: 'utf-8',
        cwd: tmpHome,
...
describe('gstack-skill-end', () => {
  ...
  test('cleans the pending analytics marker for the session', () => {
    ...
    runScript(END, ['--skill', 't', '--outcome', 'abort', '--session-id', 'sid-2', '--tel-start', 'bogus'], {
      encoding: 'utf-8',
      cwd: tmpHome,
      env: { PATH: process.env.PATH!, HOME: tmpHome, GSTACK_HOME: tmpGstackHome },
    });
