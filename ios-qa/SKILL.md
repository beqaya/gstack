...
```bash
~/.claude/skills/gstack/bin/gstack-playbook render ios-qa 2>/dev/null || true
```
...
2. Show the user the accessor list and ask whether to install the DebugBridge
   SPM dependency into their `Package.swift` (one AskUserQuestion). Record the
   answer as `$DEBUGBRIDGE_APPROVED` (yes/no) — Phase 2 step 2 gates on it.
   If declined, `/ios-qa` cannot proceed past Phase 2 step 1: every later step
   (build, launch, daemon, vision loop) talks to the bridge this dependency
   provides. Tell the user testing requires it, then stop — do not run Phase 2
   step 2 or later.
...
1. Generate the canonical local bridge package, typed accessors, and installed
   ...
   in the app target.
2. **Only if `$DEBUGBRIDGE_APPROVED` = yes (Phase 1 step 2):** add the
   generated `DebugBridge` local SPM dependency to the app's `Package.swift`.
   If the user declined, this step and Phase 2 steps 3-7 do not run — Phase 1
   already stopped the skill. The package
