...
```bash
~/.claude/skills/gstack/bin/gstack-playbook render ios-design-review 2>/dev/null || true
```
...
2. For each major screen, driven from a screen list the user provides (ask the
   user to enumerate the screens/flows in scope if they haven't already —
   there is no automated screen-discovery mechanism):
   - `GET /screenshot`
   - `GET /elements`
