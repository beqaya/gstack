...
_BRAIN_RESTORE_BIN="$HOME/.claude/skills/gstack/bin/gstack-brain-restore"
...
```bash
~/.claude/skills/gstack/bin/gstack-playbook render land-and-deploy 2>/dev/null || true
```
...
Then run `~/.claude/skills/gstack/bin/gstack-diff-scope` to classify the changes:
...
3. If SCOPE_DOCS is the only scope that's true (no frontend, no backend, no config): skip verification entirely. Tell the user: "This was a docs-only change — nothing to deploy or verify. You're all set." Go to Step 8.5 with status HEALTHY (docs-only).
...
run Step 8.5's outcome gate against the staging target, then print the deploy
report (Step 9) with verdict "STAGING VERIFIED — production deploy pending."
...
If all pass: Tell the user "Site is healthy. Page loaded in {X}s, no console errors, content looks good. Screenshot saved to {path}." Mark as HEALTHY, continue to Step 8.5.
...
After a successful revert: Tell the user "Revert pushed to {base}. The deploy should roll back automatically once CI passes. Keep an eye on the site to confirm." Note the revert commit SHA and continue to Step 8.5 with status REVERTED.

---

## Step 8.5: Outcome gate (mandatory)

**Every path into Step 9 passes through here first — there is no jump that
skips this step.** Handle the entry status you arrived with:

- **Status is REVERTED:** there is no live outcome left to prove — the
  change was deliberately rolled back and the previous version is what's
  running now. Do NOT invoke `/verify-outcome` to demand proof that the
  reverted code "works." Record `N/A (reverted — nothing to verify)` as this
  step's result, do not block, and continue straight to Step 9 with status
  REVERTED unchanged.

- **Status is HEALTHY and the scope was SCOPE_DOCS-only (Step 5 sub-step 3):**
  there is no user-visible runtime outcome to name — that's exactly why
  Step 5 skipped canary verification. Do NOT force `/verify-outcome` to
  invent a DOM/API/CLI check for a change that has no runtime effect.
  Instead, treat "no user-visible change claimed" as the outcome statement
  itself: confirm (re-check, don't just trust Step 5's classification) that
  the diff is genuinely docs-only, record that confirmation as this step's
  `PROVEN`-equivalent result, and continue to Step 9.

- **Every other case (production or staging canary reported HEALTHY, or the
  user chose "that's expected" after a DEGRADED canary):** invoke
  `/verify-outcome` for the user-visible thing this deploy (against
  whichever target Steps 6-7 just ran — staging or production) was supposed
  to make true. A `PROVEN` verdict is required for Step 9 to report
  `DEPLOYED AND VERIFIED` (or `STAGING VERIFIED`, for the staging-only path
  in Step 5a). On `UNPROVEN` or `CONTRADICTED`, Step 9 must report exactly
  that instead — including what evidence was missing — and the VERDICT line
  must NOT read `DEPLOYED AND VERIFIED` / `STAGING VERIFIED`. Step 7's
  canary checks (200 status, no console errors) are inputs to this gate,
  not substitutes for it: a green canary, a successful merge, and a passing
  CI run are explicitly not sufficient on their own (see the 2026-08-03
  blank-page outage, where all three were green while the site rendered
  nothing).

**Continue to Step 9** in every case above — Step 9 composes its VERDICT
line from whatever this step just recorded.
