...
```bash
~/.claude/skills/gstack/bin/gstack-playbook render retro 2>/dev/null || true
```
...
### Step 1.5: Features-shipped inputs (network + file reads)

`gstack-retro-metrics` is local-reads-only by design, so the two inputs the
**Features shipped** metric needs come from this separate fence. `<since>` here is
the date-only portion of the midnight-aligned start (YYYY-MM-DD, drop the
`T00:00:00` suffix) since gh's search filter is date-only:

```bash
# A. Merged PR titles in window (requires `gh` CLI + auth; empty when unavailable)
...
# B. CHANGELOG.md (file may not exist)
...
- Merged PR titles from Step 1.5 command A (`gh pr list --state merged ...`). Each
  line is `number|title|mergedAt`. Count distinct PRs and use the titles as the
  feature list. If `gh` isn't installed or isn't authenticated, command A returns
  nothing — fall back to the PR numbers in `PR_REFS` (grepped from commit
  subjects) and infer titles from the associated `COMMIT:` subjects instead.
- CHANGELOG.md entries from Step 1.5 command B. If the file exists, take the entries
...
to `PRS_REFERENCED`, and note in the narrative: "Features-shipped count is
PR-number-only — no CHANGELOG.md and no `gh` access in this repo." Never silently
show "N/A" for this row; it is the lead metric.

**Version range:** straight from `VERSION_RANGE`. If the script emitted no
`VERSION_RANGE` line, the repo has no tracked VERSION file — skip the row.
