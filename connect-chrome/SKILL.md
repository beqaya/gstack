...
"$_SS" --skill "connect-chrome" --model "claude" --parent-pid "$PPID" \
...
```bash
~/.claude/skills/gstack/bin/gstack-playbook render connect-chrome 2>/dev/null || true
```
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"connect-chrome","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/gstack/bin/gstack-skill-end --skill "connect-chrome" --outcome OUTCOME \
