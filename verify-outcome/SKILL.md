...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"verify-outcome","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"verify-outcome","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/gstack/bin/gstack-playbook render verify-outcome 2>/dev/null || true
```
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"verify-outcome","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
## Step B2 -- Name the falsifier

**Step B2 -- name the falsifier.** Before recording a verdict, state the
single observation that would prove this claim WRONG, then go make that
observation. If you cannot name one, you do not understand the claim well
enough to verify it. The falsifier must be something you have NOT already
checked -- naming a check you already ran is confirmation, not
falsification.

### Worked example -- the PostHog CSP fix

Claim: "the CSP is fixed so PostHog works." Checks that had already passed:
typecheck, four green CI/deploy workflows, deploy success, the response
header visibly changed to include the PostHog origins, and the page
rendered. Every one of those is a Step A/B signal, not a falsifier.

The falsifier not yet run: open the browser console and look for CSP
refusals. That was the one check that had been skipped.

Result when finally run: five PostHog scripts were still blocked -- by a
SECOND directive, `script-src`, not the `connect-src` directive that had
actually been fixed. The fix was half done, and every other signal said
otherwise. Verdict: CONTRADICTED, not PROVEN -- five green proxy signals do
not outrank one falsifier that was never run.
