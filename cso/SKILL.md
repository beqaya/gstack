...
# "security review" / "security check" deliberately NOT claimed: those are
# org-security phrasings that must trigger the which-skill question
# (/cso vs cyber security-review vs monthly-check), not silently route here.
- "vulnerability scan"
- "run security"

Voice triggers (speech-to-text aliases): "see-so", "see so", "vulnerability scan", "run security".
...
```bash
~/.claude/skills/gstack/bin/gstack-playbook render cso 2>/dev/null || true
```
